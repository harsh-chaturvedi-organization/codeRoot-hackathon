...
export const Navbar = () => {
    ...
    const responseGoogle = async (response) => {
        ...
        axios
            .post("http://localhost:3002/vendor/login", data)
            .then((e) => {
                // console.log(e.data);
                setUser(e.data.vendor);
                localStorage.setItem("email", e.data.vendor.email);
                // changingData(e.data.productDetails);
                setProductData(e.data.productDetails);
                setLogin(true);
            })
            .catch((err) => {
                console.log(err);
    ...
    useEffect(() => {
        ...
        if (vendorEmail) {
            axios.post("http://localhost:3002/vendor/login",{
                        email: vendorEmail,
                    },
                )
                .then((e) => {
            ...
            {isLogin ? (
                ...
                <>
                    <div className="googleSampleDesine">
                        ...
                        <img
                            className="googleAvtar"
                            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ-hcsCwCe-ozBDrOgk6P2sa0WHHocfKSiNYT-8j8AYBELZQ0EbI5LvF_UVaCo6M37cHRY&usqp=CAU"
                            alt="logo"
                        />
                    </div>
